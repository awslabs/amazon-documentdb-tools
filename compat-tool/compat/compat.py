...
def process_aggregate(le, usage_map, ver):
    retval = {}
    command = yaml.safe_load(" ".join(le.split_tokens[le.split_tokens.index("command:")+2:le.split_tokens.index("planSummary:")]), Loader=yaml.FullLoader)
    p_usage_map = {}
...
def process_query(le, usage_map, ver): 
    retval = {}
    p_usage_map = {}
    query = yaml.safe_load(le.actual_query, Loader=yaml.FullLoader)
...
def process_find(le, usage_map, ver): 
    retval = {}
    p_usage_map = {}
    query = yaml.safe_load(" ".join(le.split_tokens[le.split_tokens.index("command:")+2:le.split_tokens.index("planSummary:")]), Loader=yaml.FullLoader)
...
def process_update(le, usage_map, ver): 
    retval = {}
    p_usage_map = {}
    command = yaml.safe_load(" ".join(le.split_tokens[le.split_tokens.index("command:")+1:le.split_tokens.index("planSummary:")]), Loader=yaml.FullLoader)
