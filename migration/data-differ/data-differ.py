...
def data_compare(source_uri, target_uri, db1, db2, coll1, coll2, percent, direction):
    ...
    global differences_file
    found_error = False
    print("data_compare : " + direction)
    try:
        client1 = pymongo.MongoClient(source_uri, serverSelectionTimeoutMS=5000)
    except Exception as e:
        print("\n\n Exception connecting to source : " + str(e))
    try:
        client2 = pymongo.MongoClient(target_uri, serverSelectionTimeoutMS=5000)
    except Exception as e:
        print("\n\n Exception connecting to target : " + str(e))

    if direction != None:
        if direction == "forwards":
            differences_file.write("---_ids missing in target but present in source---\n\n")
        elif direction == "backwards":
            differences_file.write("\n\n---_ids missing in source but present in target---\n\n")
        else:
            differences_file.write("\n\n---_ids present in both target and source but differing in content---\n\n")
    ...
    for document in tqdm(sampled_docs):
        ...
        if direction == "forwards" or direction == "backwards":
            ...
            if queried_doc == None:
                ...
                differences_file.write(str(document["_id"]) + "\n")
            ...
            #     Second case is there is  guranteed differences between the documents such as for example some portion of doc missing/extra portion added.
                ...
                diff = DeepDiff(
                    document, queried_doc, verbose_level=2, report_repetition=True
                )
                found_error = True
                if len(diff) == 0:
                    # append order mismatch _id to file
                    differences_file.write(
                        "Order_Difference , " + str(document["_id"]) + "\n"
                    )
    ...
    if direction != None:
        print(
            ...
            "3)  All randomly sampled documents based on your defined percentage match exactly from source to target! - {} \n\n".format(
                direction, passed_forwards, passed_backwards, passed_forwardsAgains
            )
        )
...
def get_rand_sample_docs(sample_coll, percent):
    ...
    sampled_docs = list(sample_coll.aggregate([{"$sample": {"size": sample_size}}]))
    return sampled_docs
...
if __name__ == "__main__":
    ...
    differences_file = open(time.strftime("%Y%m%d-%H%M%S") + "-differences.txt", "a")
