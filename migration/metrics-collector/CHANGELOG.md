...
### Renamed — atlas_metrics.py is now metrics_collector.py

The tool began Atlas-specific and gained a self-managed-MongoDB-on-EC2 source in
v2.1.0, so the Atlas-specific name no longer described it. The script, its
logger name, its MongoDB `appname` strings, its temp-directory prefix, and the
default output directory name (`metrics-collector-<timestamp>`) all follow.
Every flag is unchanged; only the script filename differs at the call site.

### Fixed — one documented compression-ratio default instead of four

Four fallback compression ratios disagreed with each other, and two of them sat
adjacent in the same rendered table. The Markdown sizing summary printed the
label `~3.5:1 (estimated conservative default)` and then divided by `5` on the
next line, so the figure shown was about 30% below what its own label implied.
The JSON report divided by `5` while the Cost Estimator CSV used `3.5`, so the
two artifacts disagreed for the same cluster.

Replaced with a single module-level `DEFAULT_ZSTD_RATIO = 3.5`, used at all five
sites. `3.5` was chosen rather than `5` because estimated size is data size
divided by the ratio, so a higher ratio produces a smaller instance
recommendation. Understating compression oversizes, which is the safe direction
for a sizing tool. This value is a fallback only; a real sampled ratio always
takes precedence.

### Fixed — one ISO 8601 period parser instead of two disjoint ones

`_period_days()` handled days, hours, minutes and weeks but not years.
`period_to_start()` handled days and years but not hours, minutes or weeks, and
silently substituted **30 days** for anything it could not parse. A run with
`--period PT12H` therefore asked the Atlas API for a 30-day window while the
retention guard evaluated half a day, and neither reported a problem.
`period_to_start()` also matched with an unanchored pattern, so `P14DFOO` parsed
as 14 days.

`period_to_start()` now delegates to `_period_days()`, which is the single
parser and handles `P<n>Y`, `P<n>W`, `P<n>D`, `PT<n>H` and `PT<n>M` with
anchored patterns. An unparseable period raises instead of defaulting, and
`main()` validates `--period` before any API call so the failure is a clean
usage error rather than a mid-collection traceback.

### Fixed — 11 report fields could never populate

`generate_report` requested 16 metric names that `PROCESS_METRIC_BATCHES` never
asked the Atlas API for. Five were disk metrics that resolved anyway, because
the separate `/disks` call deposits them in the same batches dictionary. The
other 11 always resolved to null, and two more (`cpu.steal`, `cpu.iowait`) were
hardcoded `None`. Confirmed against 14 previously delivered reports: `kernel`,
`iowait`, `steal`, `virtual` and `swap_used` were null in all 42 occurrences.

`wiredtiger.cache_fill_ratio_pct` is the most consequential of these. It is a
working-set signal and it had never carried a value on the Atlas path.

Every name was verified against the live Atlas Admin API before being added,
because Atlas rejects an unknown metric name with `HTTP 404
INVALID_METRIC_NAME` for the **entire batch** rather than omitting the one name.
Results:

| Requested name | Verdict | Action |
|---|---|---|
| `PROCESS_CPU_KERNEL` | valid | added to `cpu` |
| `SYSTEM_CPU_IOWAIT`, `SYSTEM_CPU_STEAL` | valid | added to `cpu`, replacing hardcoded `None` |
| `MEMORY_VIRTUAL`, `SWAP_USAGE_USED`, `EXTRA_INFO_PAGE_FAULTS` | valid | added to `memory` |
| `CACHE_FILL_RATIO`, `TICKETS_AVAILABLE_WRITE` | valid | added to `wiredtiger` |
| `QUERY_TARGETING_SCANNED_PER_RETURNED`, `QUERY_TARGETING_SCANNED_OBJECTS_PER_RETURNED` | valid | added to `query` |
| `TICKETS_AVAILABLE_READ` | wrong name, Atlas uses the plural `TICKETS_AVAILABLE_READS` | corrected, singular kept as a fallback variant |
| `OPLOG_SLAVE_LAG_MASTER_TIME` | valid, but **secondary members only** | new `SECONDARY_ONLY_METRIC_BATCHES`, requested by role |
| `CONNECTIONS_PERCENT` | does not exist in the Atlas API | report field `connections.utilization_pct` removed |

Collection goes from 25 metric names to 36, plus 3 requested only on
secondaries. The replication-lag family is role-gated because a primary exposes
136 measurement names and a secondary 139, and the three-name difference is
exactly `OPLOG_SLAVE_LAG_MASTER_TIME`, `OPLOG_REPLICATION_LAG_TIME` and
`OPLOG_MASTER_LAG_TIME_DIFF`. Requesting them against a primary would 404 the
whole `replication` batch and degrade `OPLOG_RATE_GB_PER_HOUR` to an individual
call on every run.

Validated end to end against a 3-node Atlas replica set: all 12 previously-null
fields populate on all 3 nodes, `replication.lag_sec` populates on the 2
secondaries and is correctly absent on the primary, and the control metric
`cpu.user_normalized` is unchanged.

### Documentation

- README restructured into two self-contained per-source sections, `## Atlas`
  and `## Self-managed MongoDB`, each with its own prerequisites, installation,
  preflight, run examples, options table and output description. Previously the
  Atlas path occupied seven top-level `Step N` headings and the EC2 source was
  appended at the end, so an EC2 reader passed roughly 290 lines of Atlas API
  key setup before reaching their own instructions.
- `--source`, `--aws-region` and `--samples` now appear in an options table
  rather than prose only, marked EC2-only.
- `--all` is documented as implicitly setting `--granularity PT5M --period P14D`
  and overriding explicit values for those two flags.
- `PT10S` carries its M40+ restriction in the options table, not only in the
  retention table.
- The `Known Limitations` heading no longer claims v2.0.0 while the document
...
Atlas exposes `PROCESS_NORMALIZED_CPU_USER` separately. Delivered reports show
this field at values above 100 (`avg 322.74`, `p95 583.16` on one cluster),
which is only coherent as an un-normalized sum across cores. Renaming the field
or changing the metric behind it would make new reports non-comparable with
those already delivered, so it is left alone and recorded here.
...
Measured on Atlas `cluster-testing`: before, `compression-review.py not found`;
...
- Atlas `cluster-testing` (MongoDB 7.0.40, replica set): 7 findings, coverage
  14/14 complete
...
Known gap, unchanged by this release: index-tool does not check key direction
values other than `1` / `-1`. A direction of `2` is legal in MongoDB and was
observed in customer data; whether DocumentDB accepts it is untested.
...
The standalone `ec2_metrics.py` script is now superseded by
`metrics_collector.py --source ec2`, and is retired. Use
`metrics_collector.py --source ec2` going forward.
