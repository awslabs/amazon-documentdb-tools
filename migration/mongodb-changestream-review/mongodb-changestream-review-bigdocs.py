...
def parseChangestream(appConfig):
    ...
    #with client.watch(start_at_operation_time=startTs, full_document='updateLookup', pipeline=[{'$match': {'operationType': {'$in': ['insert','update','replace','delete']}}},{'$project':{'updateDescription':0,'fullDocument':0}}]) as stream:
    with client.watch(start_at_operation_time=startTs, full_document='updateLookup', pipeline=[{'$match': {'operationType': {'$in': ['insert','update','replace','delete']}}}]) as stream:
        while stream.alive and not allDone:
            try:
                change = stream.try_next()
                change2Resume = change["_id"]
            except:
                with client.watch(resume_after=change2Resume, full_document=None, pipeline=[{'$match': {'operationType': {'$in': ['insert','update','replace','delete']}}},]) as stream2:
                    change2 = stream2.try_next()
                    print("Exception Raised")
                    print("{} | {} | {} | {}".format(change["_id"],change["clusterTime"],change["ns"],change["documentKey"]))
